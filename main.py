...
def resize_image(event):
    ...
    new_bg_img = ImageTk.PhotoImage(resized_img)

    # Обновляем изображение у виджета
    label_bg.configure(image=new_bg_img)
    label_bg.image = new_bg_img

window = tk.Tk()
window.title("Forza Game Key")
window.geometry('2600x1300')
...
# window = tk.Tk()
# window.title("Forza Horizan Game Keygen")
# window.geometry('500x300')
...
# resized_img = original_img.resize((window.winfo_width(), window.winfo_height()), Image.ANTIALIAS)
...
# # Привязываем функцию resize_image к изменению размеров окна
